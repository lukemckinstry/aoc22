...
const returnVisible = (xIdx: number, yIdx: number, arr: number[][]) => {
...
const getNumLower = (pos: number, l: number[]) => {
  let total = 0;
  while (l.length) {
    const t = l.shift();
    if (!t) {
...
const getVisibility = (xIdx: number, yIdx: number, arr: number[][]) => {
  const pos = arr[xIdx][yIdx];
  const row = arr[xIdx];
  const col = arr.map((y) => y[yIdx]);
...
const findTotalP2 = (arr: number[][]) => {
  ...
  arr.forEach((x, xIdx) => {
    x.forEach((y, yIdx) => {
      const total = getVisibility(xIdx, yIdx, arr);
      if (total > maxVis) {
        maxVis = total;
      }
    });

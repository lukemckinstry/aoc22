...
const findTotal = (arr) => {
  ...
  arr.forEach((el) => {
    ...
    const shared = h1.split('').find((l) => h2.includes(l));
    assert.equal(shared.length, 1);
    const val = isLowerCase(shared)
      ? shared.codePointAt(0) - 96
      : shared.codePointAt(0) - 64 + 26;
    total += val;
  });
  return total;
};
...
const findTotalP2 = (arr) => {
  ...
  for (let i = 0; i < arr.length; i += 3) {
    ...
    const shared = h1.split('').find((l) => h2.includes(l) && h3.includes(l));
    assert.equal(shared.length, 1);
    const val = isLowerCase(shared)
      ? shared.codePointAt(0) - 96
      : shared.codePointAt(0) - 64 + 26;
